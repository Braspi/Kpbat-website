...
<section class="min-h-[70vh] my-5" id="contact">
  ...
  <div class:hidden={alert == null} class:bg-green-400={isSuccess} class:bg-red-400={!isSuccess}
       class="m-auto my-10 text-center w-3/5 py-2 rounded-md">{alert}</div>
      ...
      <div class="text-white flex flex-col lg:flex-row p-5 -mx-4 flex-wrap w-full">
        <div class="px-4 my-4 flex-1/2">
          ...
          <input type="text" id="name"
                 class="w-full p-2 leading-relaxed text-black h-11 bg-white border-none rounded-md"
                 placeholder={$_("form.form.name")} bind:value={request.first_name}/>
        </div>
        <div class="px-4 my-4 flex-1/2">
          <label for="surname" class="font-semibold text-xs text-light">{$_("form.form.surname").toUpperCase()}</label>
          <input type="text" id="surname"
                 class="w-full p-2 leading-relaxed text-black h-11 bg-white border-none rounded-md"
                 placeholder={$_("form.form.surname")} bind:value={request.last_name}/>
        </div>
        <div class="px-4 my-4 flex-1/2">
          <label for="email" class="font-semibold text-xs text-light">{$_("form.form.email").toUpperCase()}</label>
          <input type="text" id="email"
                 class="w-full p-2 leading-relaxed text-black h-11 bg-white border-none rounded-md"
                 placeholder={$_("form.form.email")} bind:value={request.email}/>
        </div>
        <div class="px-4 my-4 flex-1/2">
          <label for="phone" class="font-semibold text-xs text-light">{$_("form.form.phone").toUpperCase()}</label>
          <input type="text" id="phone"
                 class="w-full p-2 leading-relaxed text-black h-11 bg-white border-none rounded-md"
                 placeholder={$_("form.form.phone")} bind:value={request.phone_number}/>
        </div>
        <div class="px-4 my-4 flex-2/2">
          ...
          <textarea id="message"
                    class="w-full p-2 leading-relaxed text-black h-24 bg-white border-none rounded-md resize-none"
                    placeholder={$_("form.form.message")} bind:value={request.message}/>
        </div>
        <div class="px-4 my-4 flex-2/2">
          <label for="checked"><input type="checkbox" id="checked" aria-label="checked" bind:checked={privacy}
                                      on:change={() => triedToSend = false}/> </label>
          <span class="pl-1 text-sm font-semibold">{$_("form.form.privacy")}
            <a href="https://docs.google.com/document/d/1bnxOXVHx54XLblR6ZMMVRtHaewmt3CC4fmrkdvxEdEA/edit"
               class="hover:underline text-hoverColorDark">{$_("form.form.privacy-link")}</a>
        ...
        <div class="px-4 flex-2/2">
          <button class="text-dark bg-white py-1 px-2 rounded select-none cursor-pointer hover:bg-hoverColor"
                  on:click={() => sendForm()}>{$_("form.form.submit")}</button>
        </div>
      </div>
    </div>
