...
<div id="nav" class="w-full h-24 fixed top-0 z-10 transition duration-500 md:h-[85px] {navDetachedClass} text-black flex items-center justify-between px-4 md:px-11">
    <a href="/" class="flex items-center">
     ...
     <div class="flex gap-6 items-center">
         ...
         <div class="hidden md:flex space-x-2">
             <div role="button" tabindex="0" on:click="{() => changeLang('fr')}" on:keypress="{() => changeLang('fr')}" class="w-14 h-9 p-1.5 cursor-pointer select-none hover:brightness-50 transition duration-100">
                 <img src="/icon/flags/france.webp" alt="france-flag" class="w-full h-full">
             </div>
             <div role="button" tabindex="0" on:click="{() => changeLang('en')}" on:keypress="{() => changeLang('en')}" class="w-14 h-9 p-1.5 cursor-pointer select-none hover:brightness-50 transition duration-100">
    ...
    {#if showBurgerDiv}
        <div class="fixed inset-0 w-full h-full bg-dark flex flex-col items-center justify-center space-y-7 z-30">
            <a href="/" class="text-4xl no-underline uppercase hover:text-hoverColor transition duration-100">{$_('nav.home')}</a>
            <a href="/gallery" class="text-4xl no-underline uppercase hover:text-hoverColor transition duration-100">{$_('nav.gallery')}</a>
            <a href="/#contact" class="text-4xl no-underline uppercase hover:text-hoverColor transition duration-100">{$_('nav.contact')}</a>

            <div class="text-center">
                <h2 class="text-3xl font-semibold mb-6">{$_('nav.language')}:</h2>
                <div class="flex space-x-10">
                    <div role="button" tabindex="0" on:click="{() => changeLang('fr')}" on:keypress="{() => changeLang('fr')}" class="w-20 h-20 cursor-pointer select-none hover:brightness-50">
                        <img src="/icon/flags/france.webp" alt="france-flag" class="w-full h-full">
                    </div>
                    <div role="button" tabindex="0" on:click="{() => changeLang('en')}" on:keypress="{() => changeLang('en')}" class="w-20 h-20 cursor-pointer select-none hover:brightness-50">
                        <img src="/icon/flags/united_kingdom.webp" alt="english-flag" class="w-full h-full">
                    </div>
                </div>
            </div>
