<script lang="ts">
    ...
    onMount(() => {
        const queryId = get(page).url.searchParams.get("id");
        const id = queryId == null ? 0 : parseInt(queryId);
        sectionId = id;
        section = sections.find((item: Section) => item.id === id);
    });
</script>
    ...
    <div class="grid grid-cols-1 w-[90%] md:grid-cols-3 grid-rows-1 gap-0 md:w-3/4">
      {#if section}
        <div class="text-left col-span-2 row-span-1 w-[90%]">
          ...
          <h3 class="text-xl md:text-md lg:text-xl font-light text-dark pt-2">
